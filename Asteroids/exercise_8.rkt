...
(define-struct (player game-object)
  ...
  (define (update! player)
    (when firing-engines?
      ;; Accelerate in the forward direction if engines are firing
      (set-game-object-velocity! player (posn-+ (game-object-velocity player) (posn-* 0.5 (forward-direction player) )))))
    
  ;; Add code to update position, velocity, and handle wrapping
  ;; (if your game has wrapping behavior).
  ;; render: player -> image
...
(define-struct (asteroid game-object)
  ...
  ;; Draw the asteroid
  (define (render aster)
    (circle 25 "solid" (asteroid-color aster)))
  
  ;; radius: asteroid -> number
  ;; Size of the asteroid
  (define (radius aster)
    (asteroid-radius aster)) 
  )
...
(define-struct (missile game-object)
  ...
  (define (update! missile)
    (cond
      [(< 0 (missile-lifetime missile)) (set-missile-lifetime! missile (- (missile-lifetime missile) 1))]      
      [(= 0 (missile-lifetime missile)) (destroy! missile)]))
...
(define-struct (heat-seeker missile) ()
  #:methods
  (define (update! heat-seeker)
    (when (asteroid? (closest-asteroid-to heat-seeker))
      (set-game-object-velocity! heat-seeker (posn-+ (game-object-velocity heat-seeker) (posn-* 5.5 (heading-of (closest-asteroid-to heat-seeker) heat-seeker))))
      ))
  (define (render heat)
...
(define-struct (ufo game-object) ()
  #:methods
  (define (update! ufo)
    (set-game-object-velocity! ufo (posn-+ (game-object-velocity ufo) (posn-* 1.5 (heading-of ufo the-player)))))

  (define (destroy! ufo)
    (set-game-object-position! ufo (make-posn 100 100)))
